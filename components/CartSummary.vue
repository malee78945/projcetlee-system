<template>
  <v-card class="cart-summary-card pa-5" elevation="4">
    ...
    <v-list dense class="py-0">
      ...
      <v-list-item v-for="(item, index) in bookingitems" :key="index" class="mb-2">
        ...
        <v-list-item-action class="d-flex flex-row align-center">
          <v-btn small depressed color="secondary" class="mr-2" @click="promptForDateTime(index)">
            ตั้งเวลา
          </v-btn>
    ...
    <div class="d-flex justify-space-between align-center mt-3">
      <span class="text-h6">ราคารวม</span>
...
export default {
  ...
  methods: {
    ...
    promptForDateTime(index) {
      const date = prompt('กรุณากรอกวันที่ (YYYY-MM-DD)')
      const time = prompt('กรุณากรอกเวลา (HH:MM)')
    ...
    proceedToCheckout() {
      ...
      // 2. ตรวจสอบเวลา (เช็คแบบละเอียด)
      const incomplete = this.bookingitems.some(item => {
        return !item.bookingDate || !item.bookingTime
      })

      if (incomplete) {
        alert('กรุณากรอกวันที่และเวลาสำหรับทุกรายการก่อนไปหน้าชำระเงิน')
        return
      }

      // 3. เปลี่ยนหน้าไปที่หน้า checkout.vue
